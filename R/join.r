...
join_mutate <- function(x, y, by, type,
  ...
  keys <- common_by(by, x_in, y_in)

  join_rows <- map(keys, function(x) join_rows(x, na_equal = na_equal))
  rows <- as.list(reduce(map(join_rows, as_tibble), dplyr::intersect))

  ## not inner join
  x_raw_rowid <- seq_len(vec_size(x_in))
  y_row_rowid <- seq_len(vec_size(y_in))
  if (type == "left" || type == "full") {
    x_extra <- x_raw_rowid[!vec_in(x_raw_rowid, rows$x)]
    rows$x <- vec_c(rows$x, x_extra)
    rows$y <- vec_c(rows$y, rep(NA_integer_, length(x_extra)))
  }
  y_extra <- integer()
  if (type == "right" || type == "full") {
    y_extra <- y_row_rowid[!vec_in(y_row_rowid, rows$y)]
    rows$x <- vec_c(rows$x, rep(NA_integer_, length(y_extra)))
    rows$y <- vec_c(rows$y, y_extra)
  }

  vars <- join_cols(tbl_vars(x), tbl_vars(y), by = keys[[1]]$by, suffix = suffix, keep = keep)
  x_out <- set_names(x_in[vars$x$out], names(vars$x$out))
  y_out <- set_names(y_in[vars$y$out], names(vars$y$out))

  keep_order <- order(rows$x)
  out <- vec_slice(x_out, rows$x[keep_order])
  out[names(y_out)] <- vec_slice(y_out, rows$y[keep_order])

  if (!keep && (class(by) != 'quosures')) {
    x_key <- keys$equ$x_key
    y_key <- keys$equ$y_key
    key_type <- vec_ptype_common(x_key, y_key)
    out[names(x_key)] <- vec_cast(out[names(x_key)], key_type)

    if (length(y_extra) > 0L) {
      new_rows <- length(rows$x) - length(y_extra) + seq_along(y_extra)
      out[new_rows, names(y_key)] <- vec_cast(vec_slice(y_key, y_extra), key_type)
    }
  }
