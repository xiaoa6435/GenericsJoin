...
# unequal join ---------------------------------------------------------------
test_that("unequal join power by join by", {

  ## equal join
  df1 <- tibble(a = c(1, 2, 2, 3), b = c(2, 1, 3, 4))
  df2 <- tibble(a = c(1, 1, 2, 4), b = c(2, 2, 2, 3))

  out <- inner_join(df1, df2, join_by(a == a))
  expect_equal(out$a.x, c(1, 1, 2, 2))
  expect_equal(out$a.y, c(1, 1, 2, 2))

  out <- full_join(df1, df2, join_by(a == a))
  expect_equal(out$a.x, c(1, 1, 2, 2, 3, NA_integer_))
  expect_equal(out$a.y, c(1, 1, 2, 2, NA_integer_, 4))

  ## enhanced equal join
  out <- inner_join(df1, df2, join_by(a == a + 1))
  expect_equal(out$a.x, c(2, 2, 2, 2, 3))
  expect_equal(out$a.y, c(1, 1, 1, 1, 2))

  out <- inner_join(df1, df2, join_by(a + b == b))
  expect_equal(out$a.x, c(1, 2))
  expect_equal(out$b.y, c(3, 3))

  ## simple inequal join
  out <- inner_join(df1, df2, join_by(a > a))
  expect_equal(out$a.x, c(2, 2, 2, 2, 3, 3, 3))
  expect_equal(out$a.y, c(1, 1, 1, 1, 1, 1, 2))

  out <- full_join(df1, df2, join_by(a > a))
  expect_equal(out$a.x, c(1, 2, 2, 2, 2, 3, 3, 3, NA_integer_))
  expect_equal(out$a.y, c(NA_integer_, 1, 1, 1, 1, 1, 1, 2, 4))

  out <- left_join(df1, df2, join_by(a > a + 1))
  expect_equal(out$a.x, c(1, 2, 2, 3, 3))
  expect_equal(out$a.y, c(NA_integer_, NA_integer_, NA_integer_, 1, 1))

  out <- inner_join(df1, df2, join_by(a > a, b > b))
  expect_equal(out$a.x, c(2, 2, 3, 3, 3))
  expect_equal(out$a.y, c(1, 1, 1, 1, 2))
  expect_equal(out$b.x, c(3, 3, 4, 4, 4))
  expect_equal(out$b.y, c(2, 2, 2, 2, 2))

  ## complex inequal join
  out <- inner_join(df1, df2, join_by(a.x * a.y > 2 * b.y))
  expect_equal(out$a.x, c(2, 2, 3, 3))
  expect_equal(out$a.y, c(4, 4, 2, 4))
  expect_equal(out$b.y, c(3, 3, 2, 3))
